...
export default function App() {
  ...
  const Card = ({ children, style = {}, glow = false }) => (
    <div style={{ background: "#fff", border: `0.5px solid ${glow ? accent : "#e8e0dc"}`, borderRadius: 16, padding: "16px 18px", marginBottom: 10, ...(glow ? { boxShadow: `0 0 0 1px ${accent}22` } : {}), ...style }}>{children}</div>
  );
  ...
  const Btn = ({ children, onClick, disabled, variant = "primary", style = {} }) => {
    ...
    return <button onClick={onClick} disabled={disabled} style={{ padding: "14px 18px", borderRadius: 12, fontSize: 15, fontWeight: 600, cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.5 : 1, width: "100%", letterSpacing: 0.3, ...styles[variant], ...style }}>{children}</button>;
  };
  ...
  const Input = ({ label, ...props }) => (
    <div style={{ marginBottom: 14 }}>
      ...
      <input {...props} style={{ width: "100%", padding: "12px 14px", borderRadius: 12, border: "1px solid #e8e0dc", fontSize: 16, background: "#fff", color: "#333", boxSizing: "border-box", outline: "none", ...props.style }} />
    </div>
  );
    ...
    <div style={{ fontFamily: "'Segoe UI', -apple-system, sans-serif", color: "#333", maxWidth: 520, margin: "0 auto", padding: "0 12px 2rem", background: "#FDF8F5", minHeight: "100vh" }}>
      ...
      {showLinksModal && <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
        <div style={{ background: "#fff", borderRadius: 20, padding: "28px 24px", maxWidth: 360, width: "100%", textAlign: "center" }}>
          ...
          <a href={showWaLink} target="_blank" rel="noopener noreferrer" style={{ display: "block", padding: "14px", borderRadius: 12, background: "#25D366", color: "#fff", fontSize: 15, fontWeight: 600, textDecoration: "none", marginBottom: 10 }}>💬 Send WhatsApp Confirmation</a>
          <a href={showCalLink} target="_blank" rel="noopener noreferrer" style={{ display: "block", padding: "14px", borderRadius: 12, background: "#4285F4", color: "#fff", fontSize: 15, fontWeight: 600, textDecoration: "none", marginBottom: 10 }}>📅 Add to Google Calendar</a>
      ...
      <div style={{ marginTop: 8 }}>
        ...
        {tab === "book" && <div>
          <SectionTitle sub={editId ? "Update this appointment ✏️" : "Create a new appointment 🌸"}>{editId ? "Edit Booking" : "New Booking"}</SectionTitle>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <input ref={phoneRef} style={{ width: "100%", padding: "12px 14px", borderRadius: 12, border: "1px solid #e8e0dc", fontSize: 16, background: "#fff", color: "#333", boxSizing: "border-box", outline: "none" }} placeholder="10-digit number" type="tel" inputMode="numeric" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" onBlur={e => { const v = e.target.value.replace(/[^0-9+]/g, ""); if (v.replace(/\D/g, "").length >= 10) fillFromPhone(v); }} />
          </div>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <input ref={nameRef} style={{ width: "100%", padding: "12px 14px", borderRadius: 12, border: "1px solid #e8e0dc", fontSize: 16, background: "#fff", color: "#333", boxSizing: "border-box", outline: "none" }} placeholder="Enter full name" type="text" autoComplete="off" autoCorrect="off" spellCheck="false" />
          </div>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: "#888", marginBottom: 6, display: "block", textTransform: "uppercase", letterSpacing: 0.8 }}>Date 📅</label>
            <input style={{ width: "100%", padding: "12px 14px", borderRadius: 12, border: "1px solid #e8e0dc", fontSize: 16, background: "#fff", color: "#333", boxSizing: "border-box", outline: "none" }} type="date" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} />
          </div>
        ...
        {tab === "reminders" && <div>
          <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
            {[["rem", "Reminders 🔔"], ["thankyou", "Thank You 💖"], ["inactive", "Follow-up 📩"], ["bday", "Birthdays 🎂"]].map(([k, l]) => (
              <button key={k} style={{ flex: 1, padding: "10px", borderRadius: 12, border: (subTab === k || (!subTab && k === "rem")) ? `2px solid ${accent}` : "1px solid #e8e0dc", background: (subTab === k || (!subTab && k === "rem")) ? accentLight : "transparent", color: (subTab === k || (!subTab && k === "rem")) ? accentDark : "#999", fontSize: 11, fontWeight: 600, cursor: "pointer" }} onClick={() => setSubTab(k)}>{l}</button>
            ))}
          </div>
              ...
              return <>
                ...
                {confirmedToday.map(b => (
                  <Card key={b.id} glow>
                    ...
                    <Btn style={{ padding: "10px", fontSize: 13 }} onClick={() => completeBooking(b)}>✓ Mark Done & Send Thank You 💖</Btn>
                  </Card>
                ))}
          ...
          {subTab === "inactive" && <div>
            ...
            {inactiveClients.map(c => {
              ...
              return <Card key={c.phone}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}><span style={{ fontWeight: 700, fontSize: 15, fontFamily: "'Georgia', serif" }}>{firstName(c.name)}</span><span style={{ fontSize: 12, color: accent }}>{days}d ago</span></div><div style={{ fontSize: 13, color: "#888", marginBottom: 10 }}>{c.visits} visits · ₹{c.totalSpent} total</div><MiniBtn onClick={() => openWA(c.phone, getBookAgainMsg(c))}>Send "we miss you" message 💕</MiniBtn></Card>;
            })}
          </div>}
        ...
        {tab === "expenses" && <div>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
            ...
            <Input placeholder="₹ Amount" type="number" inputMode="numeric" value={expForm.amount} onChange={e => setExpForm({ ...expForm, amount: e.target.value })} style={{ flex: 1 }} />
          </div>
          <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
